...
public class ModelDemo
{
    ...
    public static void print(UppaalSystem sys, SystemState s) {
        System.out.print("(");
        for (SystemLocation l: s.getLocationVector()) {
	...
	if ("Linux".equals(os)) {
	    ...
	    path = here+"\\bin-Win32\\server.exe";
	}
    ...
    public static ArrayList<Transition> symbolicSimulation(Engine engine, 
							   UppaalSystem sys)
	throws EngineException, IOException
    {
	ArrayList<Transition> trace = new ArrayList<Transition>();
	// compute the initial state:
	SystemState state = engine.getInitialState(sys);
	// add the initial transition to the trace:
	trace.add(new Transition(null, null, state));
	while (state != null) {
	    print(sys, state);
	    // compute the successors (including "deadlock"):
	    ArrayList<Transition> trans = engine.getTransitions(sys, state);
	    // select a random transition:
	    int n = (int)Math.floor(Math.random()*trans.size());
	    Transition tr = trans.get(n);
    ...
    public static void saveXTRFile(ArrayList<Transition> trace, String file) 
	throws IOException 
    {
	...
	*/
	...
	out.write(it.next().getTarget().toXTRFormat());
	while (it.hasNext()) {
	    out.write(it.next().toXTRFormat());
	}
	out.write(".\n");
	out.close();	
    }
    ...
    public static void main(String[] args) {
	    ...
	    String smcsim = "simulate 1 [<=30] { v, x, y }";
	...
	new QueryFeedback() {
	    ...
	    @Override
                public void setSystemInfo(long vmsize, long physsize, 
					  long swapsize) 
	    {
	    }
	    ...
	    @Override
                public void setTrace(char result, String feedback, 
				     ArrayList<Transition> trace, int cycle) 
	    {
	    }
